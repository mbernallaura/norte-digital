...
const Sidebar = () => {
    ...
    const [selectedOption, setSelectedOption] = useState("Home"); // Estado para la opción seleccionada
        ...
        <main className={`${open ? "w-60" : "w-28"} h-screen pt-24 relative font-openSans duration-300 bg-blueLight`}>
            ...
            <ul>
                {sidebar.map((sidebarItem, index) => (
                    <Link key={index} href={sidebarItem.link}>
                        <li
                            className={`p-10 left-10 flex items-center text-center text-2xl gap-x-4 cursor-pointer font-semibold text-white hover:bg-blueDarksito ${
                                selectedOption === sidebarItem.name && "bg-blueDarksito"
                            }`}
                            onClick={() => setSelectedOption(sidebarItem.name)} 
                        >
                            {sidebarItem.src}
                            <span className={`${!open && "hidden origin-left duration-200"}`}>{sidebarItem.name}</span>
                        </li>
                    </Link>
                ))}
            </ul>
